# Modern LMS Module for Odoo

A comprehensive Learning Management System module for Odoo with modern UI/UX design.
...
To fully implement the dashboard functionality, you need to create a model called `lms.dashboard` with the following fields:
...
Don't forget to add this model in your `ir.model.access.csv` file for proper access rights.

### Init Data

To ensure there is always a dashboard record available, add this to your module's `init` method:

```python
def _ensure_dashboard_exists(self):
    dashboard = self.env['lms.dashboard'].search([], limit=1)
    if not dashboard:
        self.env['lms.dashboard'].create({'name': 'LMS Dashboard'})
```

## UI/UX Improvements

This module modernizes the standard Odoo views with:

- Card-based design for better visual hierarchy
- Modern kanban views with responsive layout
- Dashboard widgets with animated hover effects
- Improved form layouts with logical grouping
- Status indicators and visual cues for user actions
- Icon-based navigation for better usability
- Color-coded indicators for different statuses

## Credits

Developed by CS4LIBERIA
